...
bersih.split("\n").forEach((baris, nomor) => {
  ...
  if (kurungBuka > 0 && kurungTutup === 0) {
    if (!t.replace(/[{};]/g, "").trim()) {
      masalah.push(
        `baris ${nomor + 1}: blok dibuka tanpa nama directive di depannya`,
      );
    }
    return;
  }
  ...
  if (titikKoma === 0) {
    masalah.push(
      `baris ${nomor + 1}: "${t.slice(0, 50)}" tidak diakhiri titik koma`,
    );
    return;
  }
...
if (totalBuka !== totalTutup) {
  masalah.push(
    `kurung kurawal tidak seimbang, ${totalBuka} buka dan ${totalTutup} tutup`,
  );
}

const proxy = [...bersih.matchAll(/proxy_pass\s+([^;]+);/g)].map((m) =>
  m[1].trim(),
);
...
if (proxy.some((p) => p.startsWith("$")) && !adaResolver) {
  masalah.push(
    "proxy_pass memakai variabel tanpa directive resolver, nama upstream tidak akan terselesaikan",
  );
}
...
console.log(
  `proxy_pass   : ${proxy.length > 0 ? proxy.join(", ") : "(tidak ada)"}`,
);
